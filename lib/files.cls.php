...
class files
{
    ...
    public function base64Img($imgurl,$fullpath = true)
    {
        if(!file_exists($imgurl))return false;
        $ext = strtolower($this->getFileExtName($imgurl));
        if(!in_array($ext,array('jpg','png','bmp')))return false;
        if($fullpath)
        {
            return "data:image/{$ext};base64,".base64_encode(file_get_contents($imgurl));
        }
        else
        {
            return base64_encode(file_get_contents($imgurl));
        }
    }
}
?>