{x2;if:!$userhash}
{x2;include:header}
<body>
{x2;include:nav}
<div class="container-fluid">
	<div class="row-fluid">
		<div class="main">
			<div class="col-xs-2 leftmenu">
				{x2;include:menu}
			</div>
			<div id="datacontent">
{x2;endif}
				<div class="box itembox" style="margin-bottom:0px;border-bottom:1px solid #CCCCCC;">
					<div class="col-xs-12">
						<ol class="breadcrumb">
							<li><a href="index.php?{x2;$_app}-master">{x2;$apps[$_app]['appname']}</a></li>
							<li><a href="index.php?{x2;$_app}-master-basic">考场管理</a></li>
							<li class="active">成绩统计</li>
						</ol>
					</div>
				</div>
				<div class="box itembox" style="padding-top:10px;margin-bottom:0px;">
					<h4 class="title" style="padding:10px;">
						【{x2;$basic['basic']}】成绩统计
						<a class="ajax pull-right btn btn-primary" href="index.php?{x2;$_app}-master-basic-outscore&basicid={x2;$basicid}{x2;$u}">导出成绩</a>
					</h4>
					<form action="index.php?{x2;$_app}-master-basic-scorelist&basicid={x2;$basicid}" method="post">
						<table class="table form-inline">
					        <tr>
								<td style="border-top:0px;">
									考试时间：
								</td>
								<td style="border-top:0px;">
									<input class="form-control datetimepicker" data-date="{x2;date:TIME,'Y-m-d'}" data-date-format="yyyy-mm-dd" type="text" name="search[stime]" size="10" id="stime" value="{x2;$search['stime']}"/> - <input class="form-control datetimepicker" data-date="{x2;date:TIME,'Y-m-d'}" data-date-format="yyyy-mm-dd" size="10" type="text" name="search[etime]" id="etime" value="{x2;$search['etime']}"/>
								</td>
								<td style="border-top:0px;">
									分数段：
								</td>
								<td style="border-top:0px;">
									<input class="form-control" name="search[sscore]" id="sscore" type="text" value="{x2;$search['sscore']}"/> - <input class="form-control" type="text" name="search[escore]" id="escore" value="{x2;$search['escore']}"/>
								</td>
								<td style="border-top:0px;">
									批次：
								</td>
								<td style="border-top:0px;">
									<input class="form-control" name="search[ehbatch]" id="ehbatch" type="text" value="{x2;$search['ehbatch']}"/>
								</td>
							</tr>
					        <tr>
					        	<td>
									试卷：
								</td>
								<td>
									<select name="search[examid]" class="form-control">
								  		<option value="0">不限</option>
								  		{x2;tree:$exampaper,ep,eid}
								  		<option value="{x2;v:ep['examid']}"{x2;if:$search['examid'] == v:ep['examid']} selected{x2;endif}>{x2;v:ep['exam']}</option>
								  		{x2;endtree}
							  		</select>
								</td>
								<td>
								</td>
								<td colspan="4"><button class="btn btn-primary" type="submit">提交</button> <a class="btn btn-primary" href="index.php?exam-master-basic-stats&basicid={x2;$basicid}{x2;$u}">统计</a></td>
					        </tr>
						</table>
						<div class="input">
							<input type="hidden" value="1" name="search[argsmodel]" />
						</div>
					</form>
			        <table class="table table-hover table-bordered">
						<thead>
							<tr class="info">
			                    <th>ID</th>
			                    <th>名次</th>
			                    <th>考生用户名</th>
			                    {x2;tree:$fields,field,fid}
			                    <th>{x2;v:field['fieldtitle']}</th>
			                    {x2;endtree}
			                    <th>分数</th>
			                    <th>批次</th>
						        <th>考试名称</th>
						        <th>考试时间</th>
						        <th>考试用时</th>
						        <th>试卷状态</th>
								<th width="120">评卷记录</th>
								<th width="240">操作</th>
			                </tr>
			            </thead>
			            <tbody>
		                    {x2;tree:$exams['data'],exam,eid}
					        <tr{x2;if:v:exam['ehneedresit']} class="text-danger"{x2;endif}>
								<td>
									{x2;v:exam['ehid']}
								</td>
								<td>
									{x2;eval: echo ($page - 1) * 10 + v:eid}
								</td>
								<td>
									{x2;v:exam['ehusername']}
								</td>
								{x2;tree:$fields,field,fid}
			                    <th>{x2;v:exam[v:field['field']]}</th>
			                    {x2;endtree}
								<td>
									{x2;v:exam['ehscore']}
								</td>
								<td>
									{x2;v:exam['ehbatch']}
								</td>
								<td>
									{x2;v:exam['ehexam']}
								</td>
								<td>
									{x2;date:v:exam['ehstarttime'],'Y-m-d H:i'}
								</td>
								<td>
									{x2;if:v:exam['ehtime'] >= 60}{x2;if:v:exam['ehtime']%60}{x2;eval: echo intval(v:exam['ehtime']/60)+1}{x2;else}{x2;eval: echo intval(v:exam['ehtime']/60)}{x2;endif}分钟{x2;else}{x2;v:exam['ehtime']}秒{x2;endif}
								</td>
								<td>
									{x2;if:v:exam['ehstatus']}
										{x2;if:v:exam['ehneedresit']}
											已重考
										{x2;else}
											已评卷
										{x2;endif}
									{x2;else}
									待评卷
									{x2;endif}</td>
								<td>
									<a class="selfmodal btn" href="javascript:;" data-target="#modalinfo" url="index.php?exam-master-basic-resetinfo&ehid={x2;v:exam['ehid']}&useframe=1"><em class="glyphicon glyphicon-list-alt" title="评卷记录"></em></a>
								</td>
								<td>
									<div class="btn-group">
										{x2;if:v:exam['ehteacher'] && v:exam['ehstatus'] == 1 && !v:exam['ehneedresit']}
										<a class="selfmodal btn" href="javascript:;" data-target="#modal" url="index.php?exam-master-basic-resetexamhistroy&ehid={x2;v:exam['ehid']}&useframe=1">重批</a>
										{x2;endif}
										{x2;if:!v:exam['ehneedresit'] && v:exam['ehtype'] == 2 && v:exam['ehstatus']}
										<a class="btn confirm" msg="设为补考后本次成绩将作废，用户可以再进行一次考试，确定要进行此操作吗？" href="index.php?exam-master-basic-setresit&ehid={x2;v:exam['ehid']}">补考</a>&nbsp;
										{x2;endif}
										<a class="btn" href="index.php?exam-master-basic-readpaper&page={x2;$page}&ehid={x2;v:exam['ehid']}{x2;$u}" title="阅卷">阅卷</a>
										<a class="btn confirm" message="您将删除本次考试记录且无无法恢复，系统也不保存日志记录，确定要删除吗？" href="index.php?exam-master-basic-delhistory&ehid={x2;v:exam['ehid']}" title="删除">删除</a>
									</div>
								</td>
					        </tr>
					        {x2;endtree}
			        	</tbody>
			        </table>
			        <ul class="pagination pull-right">
			            {x2;$exams['pages']}
			        </ul>
				</div>
			</div>
{x2;if:!$userhash}
		</div>
	</div>
</div>
<div id="modal" class="modal fade">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button aria-hidden="true" class="close" type="button" data-dismiss="modal">×</button>
				<h4 id="myModalLabel">
					重新评卷
				</h4>
			</div>
			<div class="modal-body" id="modal-body">

			</div>
		</div>
	</div>
</div>
<div id="modalinfo" class="modal fade">
	<div class="modal-dialog" style="width: 1000px;">
		<div class="modal-content">
			<div class="modal-header">
				<button aria-hidden="true" class="close" type="button" data-dismiss="modal">×</button>
				<h4 id="myModalLabel">评卷记录</h4>
			</div>
			<div class="modal-body" id="modal-body">

			</div>
		</div>
	</div>
</div>
{x2;include:footer}
</body>
</html>
{x2;endif}