	...
	private function fields()
	{
		...
		if($this->ev->get('modifyfieldsequence'))
		{
			...
			$message = array(
			    ...
			    "forwardUrl" => "index.php?enroll-master-module-fields&moduleid={$moduleid}"
			);
		...
		if($this->ev->get('insertfield'))
		{
			...
			if($id)
			{
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "index.php?enroll-master-module-fields&moduleid={$moduleid}&page={$page}"
				);
			}
			else
			{
	...
	private function modifyfield()
	{
		if($this->ev->get('modifyfieldhtml'))
		{
			...
			$message = array(
				...
				"message" => "操作成功",
			    "callbackType" => "forward",
			    "forwardUrl" => "index.php?enroll-master-module-fields&moduleid={$field['fieldmoduleid']}"
			);
			\PHPEMS\ginkgo::R($message);
		}
	...
	private function modify()
	{
		...
		if($this->ev->get('modifymodule'))
		{
			...
			$message = array(
				...
				"message" => "操作成功",
			    "callbackType" => "forward",
			    "forwardUrl" => "index.php?enroll-master-module"
			);
			\PHPEMS\ginkgo::R($message);
		}
		else
		...
		if($this->ev->post('insertmodule'))
		{
			...
			if(!$errmsg)
			{
				$message = array(
					...
					"message" => "操作成功",
					"callbackType" => "forward",
				    "forwardUrl" => "index.php?enroll-master-module&page={$page}"
				);
				\PHPEMS\ginkgo::R($message);
			}
			else
